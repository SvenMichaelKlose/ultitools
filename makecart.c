...
static addr_t firstAddr;
/** address of previous menu entry */
static addr_t menuPrev;
 ...
 */
static int
allocate (addr_t addr, size_t size)
{
 ...
 */
static void
encodeAddr (octet_t* buf, addr_t addr)
{
  assert (addr < (1 << 24));
 ...
 */
static void
updateOffset (addr_t dest, addr_t addr)
{
 ...
 */
static addr_t
writeSubmenu (struct menuent* ent, addr_t addr, unsigned lineno)
{
  const size_t size = 3 + sizeof *ent;
  addr_t nextAddr = firstAddr + size;
...
main (int argc, char** argv)
{
  ...
  /* Insert the payload. */
  firstAddr = bitmap_offset + bitmap_size + 3;
  processFile (stdin);
