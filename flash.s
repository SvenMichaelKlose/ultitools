...
;;; @author Marko M√§kel√§ (marko.makela@iki.fi)
...
;;; Copyright © 2003,2010-2012,2015-2016 Marko M‰kel‰ (marko.makela@iki.fi)
;;;
...
id_ok	lda autosel_lo,x
	...
	lda ultimem_blk3
	sta ultimem_blk3lo_save
	lda ultimem_blk3 + 1
	sta ultimem_blk3hi_save
	setcolor(cOK)
	lda #cTEXT
	sta txtcolor
	lda #$10
	sta ultimem_blk		; BLK3 flash ROM enable
	lda #0
	sta ultimem_blk3	; BLK3 bank select low
	sta ultimem_blk3 + 1	; BLK3 bank select high
	ldy #$f0
...
ultimem_blk_save = *-1
	sta ultimem_blk
	lda #0
ultimem_blk3lo_save = *-1
	sta ultimem_blk3
	lda #0
ultimem_blk3hi_save = *-1
	sta ultimem_blk3 + 1
	cli
	rts
...
printaddr
	...
	lda ultimem_blk3
	sta tmp
	lda ultimem_blk3 + 1	; read the most significant bits of the address
	lsr
	ror tmp
	ror tmp2
	lsr
